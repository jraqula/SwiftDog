...
internal struct DatadogAuthentication {
    ...
    private mutating func get_credentials_from_env() throws {
        guard let api_key = ProcessInfo.processInfo.environment["DATADOG_API_KEY"],
              let app_key = ProcessInfo.processInfo.environment["DATADOG_APP_KEY"] else {
    ...
    internal init() {
        do {
            ...
            do {
                try self.get_credentials_from_env()
            } catch {
